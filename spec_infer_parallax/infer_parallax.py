...
parent_label_f = '../spec_download_norm/data/training_labels_parent_apogeedr17_edr3_test.fits'
norm_spec_f = '../spec_download_norm/data/all_flux_norm_parent_apogeedr17_edr3_test.fits'
...
offset = 0.017 # mas taken from Lindegren et al. 2020 (Quasar sample: median 0.017, mean 0.021)
labels['parallax'] += offset

# Negative parallax cut
# cut_negplx = labels['parallax'] > 0
# print(f"Length of import labels after negative plx cuts: {cut_negplx.sum()}")
...
if prediction: 
    ...
    hdu = fits.open(norm_spec_f)
    fluxes = hdu[0].data[:, :, 0]
    sigmas = hdu[0].data[:, :, 1]
    fluxes = fluxes[:, cut_jk * cut_hw2 * cut_finite]
    sigmas = sigmas[:, cut_jk * cut_hw2 * cut_finite]
    # fluxes = fluxes[:, cut_jk * cut_hw2 * cut_finite * cut_negplx]
    # sigmas = sigmas[:, cut_jk * cut_hw2 * cut_finite * cut_negplx]
...
if prediction:        
    ...
    for run in range(1, 5):
        ...
        for k in range(Kfold):    
            ...
            cut_burnin = labels[train]['parallax_over_error'] > 20. # XO note: Not sure why the first run needs such a strong cut at poe 20... temporarily reducing it to 5 for testing
            # cut_burnin = labels[train]['parallax_over_error'] > 0.
        ...
        labels.write('data/training_labels_new_{0}_run{1}.fits'.format(name, run), overwrite = True)
