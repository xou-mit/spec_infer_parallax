...
training_lab = join(tbl1, tbl2, keys='APOGEE_ID')

print("Gaia eDR3 and APOGEE cross match catalogue: {} entries. ".format(len(training_lab))) 
...
for i, (fn, tel, field) in enumerate(zip(training_labels['FILE'][:3], training_labels['TELESCOPE'][:3], training_labels['FIELD'][:3])):
    ...
    if not os.path.isfile(destination):# or os.path.isfile(destination2)):
        urlbase = 'https://data.sdss.org/sas/apogeework/apogee/spectro/redux/r12/stars/' \
...
print('save labels...')
Table.write(training_labels, 'data/training_labels_parent_apogeedr16_edr3_1.fits', format = 'fits', overwrite = True)
...
file_name = 'all_flux_norm_parent_apogeedr16_edr3_1.fits'
